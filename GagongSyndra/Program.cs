...
/* 
 * delete using LXOrbwalker
 * delete dfg
 * PrintChat
 * BaseSkinName => CharData.BaseSkinName
 * Want Update Orbwalker (xSaliceResurrected_xSaliceWalker) but idk how to update it ...
 * Use E LaneClear
 * Interrupted Spells
 * clear up
 * By NightMoon
...
namespace GagongSyndra
{
    public static class Program
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Game.PrintChat("<font color = \"#FF00FF\">Updates by ScienceARK</font>");
        }

        public static void Interrupter2_OnInterruptableTarget(Obj_AI_Hero unit, Interrupter2.InterruptableTargetEventArgs spell)
        {
            dd(unit, spell);
        }

        public static void dd(Obj_AI_Hero unit, Interrupter2.InterruptableTargetEventArgs spell)
        {
            ...
            if (Spells.E.IsReady() && Player.Distance(unit, true) <= Math.Pow(Spells.E.Range, 2) && unit.IsValidTarget(Spells.E.Range))
            {
                ...
                Spells.UseQeSpell((Obj_AI_Hero)unit);
        }

        public static void SyndraMenu()
        {
            //Base menu
            Menu = new Menu("Gagong" + ChampionName, "Gagong" + ChampionName, true);
        ...
        public static void Farm()
        {
            ...
            {
                if (UseQLaneClear && UseQLastHit && !Player.IsWindingUp && Spells.Q.IsReady())
                {
        ...
        {
            {
                ...
                {
                    Spells.UseQSpell((Obj_AI_Hero)sender);
                }
        ...
        public static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (!Spells.E.IsReady() || !(Player.Distance(gapcloser.Sender, true) <= Math.Pow(Spells.QE.Range, 2)) ||
                ...
                return;
            if (Spells.E.IsReady() && Player.Spellbook.GetSpell(SpellSlot.E).ManaCost <= Player.Mana)
            {
...
/*        public static void Interrupter_OnPossibleToInterrupt(Obj_AI_Base unit, Interrupter2.InterruptableTargetEventArgs spell)
        ...
        public static void AutoKs()
        {
            ...
            foreach (
                var enemy in
                    ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.Team != Player.Team).Where(enemy =>
