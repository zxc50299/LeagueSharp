...
namespace GagongSyndra
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Q.SetSkillshot(0.85f, 125f, float.MaxValue, false, SkillshotType.SkillshotCircle);

            W = new Spell(SpellSlot.W, 925);
            W.SetSkillshot(0.25f, 140f, 1600f, false, SkillshotType.SkillshotCircle);

            E = new Spell(SpellSlot.E, 700);
            E.SetSkillshot(0.30f, (float) (45*0.5), 2500, false, SkillshotType.SkillshotCone);
            ...
            QE = new Spell(SpellSlot.E, 1300);
            QE.SetSkillshot(float.MaxValue, 65f, 8000f, false, SkillshotType.SkillshotLine);
            ...
            orbwalkerMenu = new Menu("[Gagong] Orbwalker", "[Gagong] Orbwalker");
            //TargetSelector
            TargetSelector.AddToMenu(Menu.SubMenu("[Gagong] Target Selector"));
            ...
            Menu.SubMenu("Farm").AddItem(new MenuItem("laneQ", "Use Q").SetValue(true));
            Menu.SubMenu("Farm").AddItem(new MenuItem("laneW", "Use W").SetValue(true));
            Menu.SubMenu("Farm").AddItem(new MenuItem("laneQL", "Lasthit Only [Q]").SetValue(true));
            Menu.SubMenu("Farm").AddItem(new MenuItem("lanemana", "Mana %").SetValue(new Slider(65, 100, 0)));


            //JungleFarm menu:
            Menu.AddSubMenu(new Menu("Jungle Settings", "JungleFarm"));
            ...
            Menu.SubMenu("AutoKS")
                .AddItem(
                    new MenuItem("AutoKST", "Killsteal [TOGGLE]").SetValue(new KeyBind("U".ToCharArray()[0],
                        KeyBindType.Toggle, true)));
            ...
            Menu.SubMenu("Misc")
                .AddItem(new MenuItem("IgniteALLCD", "Ignite if all spells are on cooldown").SetValue(false));
            ...
            Menu.SubMenu("QEsettings")
                .AddItem(
                    new MenuItem("UseQEC", "[QE] to Enemy Near Cursor").SetValue(new KeyBind("T".ToCharArray()[0],
                        KeyBindType.Press)));
            ...
            Menu.AddItem(new MenuItem("Info", "Updated by ScienceARK"));
        ...
        {
            if (_yasuoWall == null || !Menu.Item("YasuoWall").GetValue<bool>() ||
                !ObjectManager.Get<Obj_AI_Hero>()
        ...
        private static void Farm()
        {
            ...
            if (qfarmpos.MinionsHit >= 2 && Menu.Item("laneQ").GetValue<bool>() && !Menu.Item("laneQL").GetValue<bool>())
                Q.Cast(qfarmpos.Position);

            foreach (var minion in allMinionsQ.Where(a => a.Health <= Q.GetDamage(a) && a.IsEnemy && a.IsValid))
            {
                if (Menu.Item("laneQ").GetValue<bool>() && Menu.Item("laneQL").GetValue<bool>() && !Player.IsWindingUp)
                    Q.Cast(minion);
            }

            if (!Menu.Item("laneW").GetValue<bool>())
                return;

            if (Player.Spellbook.GetSpell(SpellSlot.W).ToggleState == 1 && allMinionsW.Count >= 2)
            {
                //WObject
        ...
        {
            {
                ...
                if (Menu.Item("HarassAAQ").GetValue<bool>() && sender.Type == Player.Type && sender.Team != Player.Team &&
                    ...
                    Player.Distance(sender, true) <= Math.Pow(Q.Range, 2) &&
        ...
        private static double GetRDamage(Obj_AI_Base enemy)
        {
        ...
        private static bool RCheck(Obj_AI_Hero enemy)
        {
            ...
            if (Menu.Item("DontRwParam").GetValue<StringList>().SelectedIndex == 0 &&
                GetComboDamage(enemy, Menu.Item("DontRwQ").GetValue<bool>(), Menu.Item("DontRwW").GetValue<bool>(),
                    Menu.Item("DontRwE").GetValue<bool>(), false, false) + aa >= enemy.Health) return false;
            //If can be killed by either any of the skills
            if (Menu.Item("DontRwParam").GetValue<StringList>().SelectedIndex == 1 &&
                (GetComboDamage(enemy, Menu.Item("DontRwQ").GetValue<bool>(), false, false, false, false) >=
                 enemy.Health ||
                 GetComboDamage(enemy, Menu.Item("DontRwW").GetValue<bool>(), false, false, false, false) >=
                 enemy.Health ||
                 GetComboDamage(enemy, Menu.Item("DontRwE").GetValue<bool>(), false, false, false, false) >=
        ...
        private static float overkillcheckv2(Obj_AI_Base target)
        {
            double dmg = 0;
            if (Q.IsReady())
            ...
            if (Player.Distance(target.Position) <= 550)
        ...
        private static void UseSpells(bool uq, bool uw, bool ue, bool ur, bool uqe)
        {
            ...
            var qeTarget = TargetSelector.GetTarget(QE.Range + 150, TargetSelector.DamageType.Magical);

            //Harass Combo Key Override
            if (rTarget != null && (harassKey.GetValue<KeyBind>().Active || laneclearKey.GetValue<KeyBind>().Active) &&
                comboKey.GetValue<KeyBind>().Active && Player.Distance(rTarget, true) <= Math.Pow(R.Range, 2) &&
                BuffCheck(rTarget) && DetectCollision(rTarget))
            {
                if (Menu.Item("DontR" + rTarget.BaseSkinName) != null &&
                    Menu.Item("DontR" + rTarget.BaseSkinName).GetValue<bool>() == false && ur && overkillcheckv2(rTarget) <= rTarget.Health && Player.HealthPercent >= 35)
                {
                    R.CastOnUnit(rTarget);
                    R.LastCastAttemptT = Environment.TickCount;
                }
            }
            if (IgniteSlot.IsReady() && qTarget.Health < GetIgniteDamage(qTarget))
                                ...
                                BuffCheck(enemy)))
                {
                        ...
                        RCheck(enemy))
                    {
                        if (
                            ...
                            Environment.TickCount - Q.LastCastAttemptT > 500 + Game.Ping && overkillcheckv2(rTarget) <= rTarget.Health && Player.HealthPercent >= 35)
                        {
                            R.CastOnUnit(enemy);
                            R.LastCastAttemptT = Environment.TickCount;
                        }
            ...
            //Use QE
            if (uqe && DetectCollision(qeTarget) && qeTarget != null && Q.IsReady() &&
                (E.IsReady() &&
                  Player.Spellbook.GetSpell(SpellSlot.E).Level > 0) &&
                Player.Spellbook.GetSpell(SpellSlot.Q).ManaCost + Player.Spellbook.GetSpell(SpellSlot.E).ManaCost <=
                Player.Mana)
            {
                UseQe(qeTarget);
            }
            ...
            if (ue && E.IsReady() && Environment.TickCount - W.LastCastAttemptT > Game.Ping + 150 &&
                ...
                {
                    if (GetComboDamage(enemy, uq, uw, ue, ur) > enemy.Health &&
                        Player.Distance(enemy, true) <= Math.Pow(E.Range, 2))
                        E.Cast(enemy);
        ...
        private static void UseQ(Obj_AI_Base target)
        {
            ...
            if (pos.Hitchance >= HitChance.VeryHigh && target.IsValidTarget(Q.Range))
        ...
        private static void UseW(Obj_AI_Base qeTarget, Obj_AI_Base wTarget)
        {
            ...
            if (qeTarget != null && W.IsReady() && Player.Spellbook.GetSpell(SpellSlot.W).ToggleState == 1)
            {
                ...
                if (gObjectPos.To2D().IsValid() && Environment.TickCount - Q.LastCastAttemptT > Game.Ping + 150 &&
                    Environment.TickCount - E.LastCastAttemptT > 750 + Game.Ping &&
                    Environment.TickCount - W.LastCastAttemptT > 750 + Game.Ping)
                {
...
//            if (wTarget == null || Player.Spellbook.GetSpell(SpellSlot.W).ToggleState != 2 || !W.IsReady())
            //               return;
            if (wTarget != null && W.IsReady() && Player.Spellbook.GetSpell(SpellSlot.W).ToggleState == 2 && wTarget.IsValidTarget(W.Range) &&
                Environment.TickCount - E.LastCastAttemptT > 300)
            {
...
//                Game.PrintChat("casting w " + Player.Spellbook.GetSpell(SpellSlot.W).ToggleState + " pred"+pos.Hitchance);
            ...
            foreach (
                var orb in
                    OrbManager.GetOrbs(true)
                        .Where(orb => orb.To2D().IsValid() && Player.Distance(orb, true) < Math.Pow(E.Range, 2)))
            {
                ...
                if (pPo.Distance(sp, ep, true, true) <= Math.Pow(QE.Width + target.BoundingRadius, 2) && Player.Distance(sp) <= E.Range)
                    E.Cast(orb);
            }
        }
        ...
        private static void UseQe2(Obj_AI_Base target, Vector3 pos)
        {
            ...
            if (!(pPo.SegmentPoint.Distance(target, true) <= Math.Pow(QE.Width + target.BoundingRadius, 2)))
                return;
            var delay = 120 - (int)(Player.Distance(pos) / 2.5) + Menu.Item("QEDelay").GetValue<Slider>().Value;
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Menu.Item("DrawQEMAP").GetValue<bool>())
            {
                ...
                if (tPos != null && Player.Distance(sPos, true) > Math.Pow(E.Range, 2) &&
                    (E.IsReady() || Player.Spellbook.GetSpell(SpellSlot.E).CooldownExpires - Game.Time < 2) &&
                    Player.Spellbook.GetSpell(SpellSlot.E).Level > 0)
                {
                    var color = Color.Red;
