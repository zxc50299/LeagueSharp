...
namespace GagongSyndra
{
    public static class GetDamage
    {
        public static float GetComboDamage(Obj_AI_Base enemy, bool UseQ, bool UseW, bool UseE, bool UseR)
        {
            ...
            {
                combomana += ObjectManager.Player.Spellbook.GetSpell(SpellSlot.R).ManaCost;
                if (combomana <= ObjectManager.Player.Mana) damage += GetRDamage(enemy);
                else combomana -= ObjectManager.Player.Spellbook.GetSpell(SpellSlot.R).ManaCost;
            }

            //Add Q Damage
            if (Spells.Q.IsReady() && UseQ)
            {
                combomana += ObjectManager.Player.Spellbook.GetSpell(SpellSlot.Q).ManaCost;
                if (combomana <= ObjectManager.Player.Mana) damage += ObjectManager.Player.GetSpellDamage(enemy, SpellSlot.Q);
                else combomana -= ObjectManager.Player.Spellbook.GetSpell(SpellSlot.Q).ManaCost;
            }

            //Add E Damage
            if (Spells.E.IsReady() && UseE)
            {
                combomana += ObjectManager.Player.Spellbook.GetSpell(SpellSlot.E).ManaCost;
                if (combomana <= ObjectManager.Player.Mana) damage += ObjectManager.Player.GetSpellDamage(enemy, SpellSlot.E);
                else combomana -= ObjectManager.Player.Spellbook.GetSpell(SpellSlot.E).ManaCost;
            }

            //Add W Damage
            if (Spells.W.IsReady() && UseW)
            {
                combomana += ObjectManager.Player.Spellbook.GetSpell(SpellSlot.W).ManaCost;
                if (combomana <= ObjectManager.Player.Mana) damage += ObjectManager.Player.GetSpellDamage(enemy, SpellSlot.W);
                else combomana -= ObjectManager.Player.Spellbook.GetSpell(SpellSlot.W).ManaCost;
            }
